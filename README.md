...
```shell
ros2 run lecture_ros2_01 basic_navigator_sample_02.py
```

`BasicNavigator`を使って、ウェイポイントで逐一停止しないようにするサンプルです。
目標地点との距離をチェックしており、一定範囲内に入ったらループを抜けて次の目標を与えます。

明らかに同じような`while`ループが重複していることが分かるでしょう。
無駄なコードを無くすにはどうしたらよいか考えてみましょう。
